...
internal class BounceShake : ShakePreset
{
    ...
    private float currentOffsetAmount, currentEulersAmount;
    ...
    internal override Displacement ExecuteShake(float delta)
    {
        ...
        var pivot = Mathf.Sin(offset);

        if (Mathf.Sign(pivot) != Mathf.Sign(pivotLastFrame))
        {
            GD.Randomize();
            var r = (GD.Randf() * 2 - 1) * jitter;
        ...
        var disp = new Displacement(currentDir * currentOffsetAmount * pivot, Vector3.Zero);
