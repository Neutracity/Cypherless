...
public struct Envelope
{
    ...
    /// </summary>
    public Degree EnvelopeDegree { get; set; } = Degree.Quadratic;
    ...
    public float Evaluate(float normalizedTime)
    {
        ...
        var xBound = 1.0f / attack;

        var envelopeLimit = xBound + Sustain + 1.0f / decay;
        var x = normalizedTime * envelopeLimit;

        if (x > 0 && x < xBound)
            return Power.Evaluate(attack * x, EnvelopeDegree);
        if (x > xBound && x < xBound + sustain)
            return 1.0f;
        if (x > xBound + sustain && x < envelopeLimit)
            return Power.Evaluate(1.0f - (x - xBound - sustain) * decay, EnvelopeDegree);
        return 0;
    }
}